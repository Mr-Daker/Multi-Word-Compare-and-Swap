module Spec = struct
  ...
  let init () = Lockfree_linked_list.create ()
  let cleanup _ = ()

  let run cmd t =
    match cmd with
    | Insert k -> RBool (Lockfree_linked_list.insert t k)
    | Delete k -> RBool (Lockfree_linked_list.delete t k)
    | Member k -> RBool (Lockfree_linked_list.member t k)
end
...
let () =
  ...
  QCheck_base_runner.run_tests_main
    [
      QCheck.Test.make ~count:10 ~retries:1 ~name:"lockfree_linked_list_linearizable"
        arb Test.lin_prop;
      QCheck.Test.make ~count:10 ~retries:1 ~name:"lockfree_linked_list_stress"
