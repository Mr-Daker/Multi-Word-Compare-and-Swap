...
IMPL_COLORS = {
    "lockfree_linked_list": "#38bdf8",
    "mcas_lockfree_linked_list": "#a78bfa",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate linked-list benchmark visualizations.")
    parser.add_argument(
        "--input-csv",
        default=str(repo_root() / "presentation" / "data" / "benchmark_linked_list_results_latest.csv"),
        help="CSV file produced by run_benchmarks_linked_list.py.",
    )
    parser.add_argument(
        "--output-dir",
...
def plot_scenario(
    ...
    output_path = output_dir / f"ll_{scenario}.png"
    fig.tight_layout()
...
def plot_overview(
    ...
    fig.suptitle("MCAS Linked-List Elapsed-Time Overview", fontsize=24, weight="bold", color="#0f172a", y=0.995)
    fig.tight_layout(rect=(0, 0, 1, 0.95))

    output_path = output_dir / "ll_benchmark_overview.png"
    fig.savefig(output_path, bbox_inches="tight")
    plt.close(fig)
    return output_path
...
def plot_relative_to_baseline(
    ...
    for impl_name in ("mcas_lockfree_linked_list",):
        ratios = []
        for scenario in scenarios:
            baseline_rows = grouped[scenario]["lockfree_linked_list"]
    ...
    ax.set_title(
        "MCAS Linked-List Elapsed-Time Slowdown at 8 Threads",
        fontsize=18,
        weight="bold",
        color="#0f172a",
        pad=14,
    ...
    output_path = output_dir / "ll_relative_to_baseline.png"
    fig.tight_layout()
    fig.savefig(output_path, bbox_inches="tight")
    plt.close(fig)
    return output_path
