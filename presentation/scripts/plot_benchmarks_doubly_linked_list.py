...
IMPL_COLORS = {
    "coarse_doubly_linked_list": "#f43f5e",
    "mcas_doubly_linked_list": "#10b981",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate doubly linked-list benchmark visualizations.")
    parser.add_argument(
        "--input-csv",
        default=str(repo_root() / "presentation" / "data" / "benchmark_doubly_linked_list_results_latest.csv"),
        help="CSV file produced by run_benchmarks_doubly_linked_list.py.",
    )
    parser.add_argument(
        "--output-dir",
...
def plot_scenario(
    ...
    output_path = output_dir / f"dll_{scenario}.png"
    fig.tight_layout()
...
def plot_overview(
    ...
    fig.suptitle("MCAS Doubly-Linked-List Elapsed-Time Overview", fontsize=24, weight="bold", color="#0f172a", y=0.995)
    fig.tight_layout(rect=(0, 0, 1, 0.95))

    output_path = output_dir / "dll_benchmark_overview.png"
    fig.savefig(output_path, bbox_inches="tight")
    plt.close(fig)
    return output_path
...
def plot_relative_to_baseline(
    ...
    for impl_name in ("mcas_doubly_linked_list",):
        ratios = []
        for scenario in scenarios:
            baseline_rows = grouped[scenario]["coarse_doubly_linked_list"]
    ...
    ax.set_title(
        "MCAS Doubly-Linked-List vs Mutex Slowdown at 8 Threads",
        fontsize=18,
        weight="bold",
        color="#0f172a",
        pad=14,
    ...
    ax.set_ylabel("Time relative to baseline (x) [Lower is Better]", fontsize=12, color="#334155")
    ax.set_xticks(xs, [SCENARIO_LABELS[scenario] for scenario in scenarios], rotation=0)
    ax.legend(frameon=False, fontsize=10, loc="upper right")

    output_path = output_dir / "dll_relative_to_baseline.png"
    fig.tight_layout()
    fig.savefig(output_path, bbox_inches="tight")
    plt.close(fig)
    return output_path
