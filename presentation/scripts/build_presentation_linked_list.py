...
IMPL_LABELS = {
    "lockfree_linked_list": "Baseline CAS linked-list",
    "mcas_lockfree_linked_list": "MCAS linked-list",
}

IMPL_COLORS = {
    "lockfree_linked_list": RGBColor(56, 189, 248),
    "mcas_lockfree_linked_list": RGBColor(167, 139, 250),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the linked list benchmark presentation deck.")
    parser.add_argument(
        "--input-csv",
        default=str(repo_root() / "presentation" / "data" / "benchmark_linked_list_results_latest.csv"),
    ...
    parser.add_argument(
        "--output",
        default=str(repo_root() / "presentation" / "mcas_linked_list_benchmark_presentation.pptx"),
...
def top_findings(grouped: dict[str, dict[str, list[dict[str, object]]]]) -> list[str]:
    ...
    baseline = float(balanced["lockfree_linked_list"][-1]["seconds"])
    volatile = float(balanced["mcas_lockfree_linked_list"][-1]["seconds"])

    search = grouped["search_100"]
    search_baseline = float(search["lockfree_linked_list"][-1]["seconds"])
    search_volatile = float(search["mcas_lockfree_linked_list"][-1]["seconds"])

    updates = grouped["update_heavy_45_45_10"]
    update_baseline = float(updates["lockfree_linked_list"][-1]["seconds"])
    update_volatile = float(updates["mcas_lockfree_linked_list"][-1]["seconds"])

    return [
        f"At 8 threads on a balanced workload, the baseline finishes in {human_ms(baseline)} while the MCAS linked list takes {human_ms(volatile)}.",
        f"For 100% searches at 8 threads, the MCAS linked list is {search_volatile / search_baseline:.1f}x slower than the CAS baseline.",
        f"Even on heavy updates at 8 threads, the MCAS layer stretches total runtime from {human_ms(update_baseline)} to {human_ms(update_volatile)}.",
    ]


def scenario_findings(grouped: dict[str, dict[str, list[dict[str, object]]]], scenario: str) -> list[str]:
    scenario_rows = grouped[scenario]
    baseline = float(scenario_rows["lockfree_linked_list"][-1]["seconds"])
    volatile = float(scenario_rows["mcas_lockfree_linked_list"][-1]["seconds"])
    baseline_avg = float(scenario_rows["lockfree_linked_list"][-1]["avg_us_per_op"])
    volatile_avg = float(scenario_rows["mcas_lockfree_linked_list"][-1]["avg_us_per_op"])
    return [
...
def build_deck(
    ...
    add_title(
        ...
        "Benchmarking and lock-free linked list performance presentation deck",
    )
    hero_box = slide.shapes.add_textbox(Inches(0.75), Inches(2.0), Inches(6.8), Inches(2.2))
    p = hero_box.text_frame.paragraphs[0]
    r = p.add_run()
    r.text = "How much simplicity does MCAS buy us for linked lists, and what does the software layer cost?"
    r.font.size = Pt(28)
    r.font.bold = True
    r.font.color.rgb = RGBColor(248, 250, 252)
    add_bullet_list(
        slide,
        [
            "Implements descriptor-based MCAS and applies it to a lock-free linked list.",
            "Benchmarks baseline CAS linked list against the MCAS linked list.",
    ...
    add_bullet_list(
        slide,
        [
            "Deliverable focus: compare linked list elapsed time across varying update/search ratios and 2-8 threads.",
            "Workloads: 90% updates, 67% updates, and 100% searches.",
            "Implementations: baseline CAS linked list and MCAS linked list.",
            "Benchmark harness runs operations uniformly per domain.",
    ...
    slide.shapes.add_picture(str(assets_dir / "ll_benchmark_overview.png"), Inches(0.7), Inches(1.55), width=Inches(8.7))
    slide.shapes.add_picture(str(assets_dir / "ll_relative_to_baseline.png"), Inches(9.55), Inches(1.9), width=Inches(3.0))
    ...
    add_bullet_list(
        slide,
        [
            "MCAS makes the linked list implementation conceptually straightforward by using multi-word operations for insertion and deletion.",
            "That expressiveness comes with a visible time cost, compared to a carefully crafted CAS-based lock-free linked list.",
