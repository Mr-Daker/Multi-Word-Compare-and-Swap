...
let benchmark_impl
  ...
  List.concat_map
    (fun scenario ->
      List.map
        (fun domains ->
          printf "running %-28s %-24s domains=%d\n" impl_name scenario.name domains;
          flush ();
          let result =
...
let () =
  print_header ();
  let cas_results =
    benchmark_impl ~impl_name:"lockfree_linked_list"
      ~create:Lockfree_linked_list.create ~insert:Lockfree_linked_list.insert
      ~delete:Lockfree_linked_list.delete ~member:Lockfree_linked_list.member
  in
  let mcas_results =
    benchmark_impl ~impl_name:"mcas_lockfree_linked_list"
      ~create:Mcas_lockfree_linked_list.create
      ~insert:Mcas_lockfree_linked_list.insert
      ~delete:Mcas_lockfree_linked_list.delete
      ~member:Mcas_lockfree_linked_list.member
  in
  print_summary_group (cas_results @ mcas_results)
